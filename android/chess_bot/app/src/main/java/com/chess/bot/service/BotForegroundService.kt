...
class BotForegroundService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        try {
            ...
            LogBus.log(LogKind.ERROR, "前台服务处理异常：${e::class.java.simpleName}: ${e.message}")
    ...
    private fun handleStart(intent: Intent?): Int {
        ...
        if (resultCode == Int.MIN_VALUE || resultData == null) {
            LogBus.log(LogKind.ERROR, "启动服务缺少屏幕捕获授权参数")
            stopSelf()
            return START_NOT_STICKY
        }
        LogBus.log(LogKind.INFO, "收到授权结果：code=$resultCode")
        ...
        LogBus.log(LogKind.INFO, "前台服务已进入 mediaProjection 类型")

        if (ScreenCaptureSource.get().start(this, resultCode, resultData)) {
            LogBus.log(LogKind.OK, "截屏管线已启动")
            com.chess.bot.overlay.OverlayManager.ensureShown(this)
        } else {
            LogBus.log(LogKind.ERROR, "截屏管线启动失败，请重新授权")
            stopSelf()
        }
        return START_NOT_STICKY
    }
