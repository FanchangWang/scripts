...
class BotSession(internal val context: Context) {
     ...
     */
    internal suspend fun recoverOwnLift(
        correctedOwned: Mat,
    ...
    internal suspend fun startFlow() {
        running = true
        emit()
        try {
            engine.newGame(context)
     ...
     */
    internal suspend fun doMove(dstOnlySrc: Pair<Int, Int>? = null): Boolean {
    ...
    internal suspend fun computeMove(): PendingMove? {
    ...
    internal fun unpackMove(move: String): Unpacked? {
    ...
    internal suspend fun attemptMove(
        r1: Int,
        c1: Int,
        r2: Int,
        c2: Int
    ...
    internal suspend fun waitForEnemyMove() {
        setStatus(BotStatus.WAIT_ENEMY)
        state.resignStreak = 0
        state.noisyCount = 0
     ...
     */
    internal suspend fun confirmEndByOcr(): Boolean {
        val now = System.nanoTime()
    ...
    internal fun updateResign(newBoard: Board, changes: List<Change>): ResignResult {
    ...
    internal fun evalDetail(r: EngineResult): String = when {
    ...
    internal fun evalOverlayText(): String = when {
        state.lastEvalScoreUnreliable -> "评估 -"
        else -> {
            val matePly = state.lastMatePly
            when {
    ...
    internal fun recordMateInfo(result: EngineResult) {
    ...
    internal suspend fun checkmateProbe(): Boolean {
    ...
    internal fun logProbeSkipChange(key: String, text: String) {
    ...
    internal fun decideDraw(): Boolean {
    ...
    internal suspend fun autoNextGame(): Boolean {
     ...
     */
    internal suspend fun reconfirmEnemyMoved(move: Move): Grabbed? {
     ...
     */
    internal fun maybeHarvestPonderCap() {
     ...
     */
    internal fun commitEnemyMove(move: Move, grab: Grabbed) {
    ...
    internal fun applyEnemyMove(move: Move) {
