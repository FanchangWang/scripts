...
- **引擎 position 自检（2026-09-12 用户批复 D2：自检 + 报错中止；真机事故 a7c5）**：新增纯函数 `auditEnginePosition(baselineFen, mySide, moves, expectedBoard, expectedTurn)`（Board.kt）+ 会话层守卫 `BotSession.auditPosition(baselineFen)`（BotSessionFlow.kt）——发 `position fen <基线> moves <全列表>` 之前，**逐手校验严格交替**（每手持子方 == 演进行棋方）**+ 伪合法 + 演进局面与已提交 `state.board`/`state.turn` 一致**；不过则打 ERROR 并中止本步。接线两处发送点：`computeMove` 引擎段（`ensureEngineBaseline()` 之后，不过 → `return null` → doMove 返回 false → 主循环暂停）、`maybeStartPonder`（不过 → 跳过本轮 ponder，不中止对局；错局面下一轮 bestMove 自检必拦）。**动因**：UCI 引擎解析 `position ... moves ...` 遇首个非法着法即停止、后续全丢 → 在**过期局面**上算棋；事故中 movesList 因「我方未提交却先提交敌着」变成同色连走，引擎返回已走过的 a7c5（该着法在 board 上仍合法，`unpackMove` 守卫拦不住）。配套 `boardFromFen`（与 `fenOfBoard` 严格互逆，仅解析本项目自产形状 `rank9/…/rank0 <w|b> …`，失败返回 null）。单测 `PositionAuditTest`（8 例，含同色连走事故形状）。
