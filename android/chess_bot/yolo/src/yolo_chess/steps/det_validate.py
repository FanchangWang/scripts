...
def main() -> int:
    ...
    for st, files, out_dir in iter_state_images(states, OUTPUT_DIR):
        ...
        if st_sum is not None:
            ...
            content += [
                ...
                f"bias={np.hypot(sd[:, :, 0].mean(), sd[:, :, 1].mean()):.3f} "
                f"std={np.hypot(sd[:, :, 0].std(), sd[:, :, 1].std()):.3f}",
    ...
    print(
        f"  全局: |bias|={g_bias:.2f}px  std={g_std:.2f}px  → "
        + (
    ...
    if sh_p95 <= SHIFT_GOOD:
        ...
        verdict = f"❌ 不合格：{why}。可提高训练 imgsz 并重训（增强建议用 --aug robust）。"
        print(f"  {verdict}")
