...
2. **棋盘四角定位（det）**：检测棋盘 4 个角点（TL/TR/BL/BR），替代固定分辨率查表矫正。
...
| det 构建数据集 | det_dataset | 由四角真值生成 YOLO 检测标签 |
...
```
yolo/
├── src/yolo_chess/        # 包：common.py（共享）+ steps/（各步骤）+ cli.py（主菜单）
...
- 导出模型（`cls/export/`、`det/export/`）与 `model_info.json` 入库提交
