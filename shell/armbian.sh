...
while true; do
    ...
    echo "3. 安装 mihomo"
    echo "4. 安装 Docker"
    echo "100. 安装 watchtower 容器"
    echo "101. 安装 allinone & allinone_format 容器"
    ...
    if [[ "$choice" -eq 3 ]]; then
        ...
        # 下载 geo 文件
        files=("GeoIP.dat" "GeoSite.dat" "GeoIP.metadb" "GeoLite2-ASN.mmdb")
            ...
            if [[ ! -f "$file" ]]; then
                ...
                case $file in
                    ...
                    "GeoIP.metadb")
                        ...
                        ;;
                    "GeoLite2-ASN.mmdb")
                        download_from_github "GeoLite2-ASN.mmdb" "https://github.com/xishang0128/geoip/releases/download/latest/GeoLite2-ASN.mmdb"
                        ;;
                esac
            fi
        done
...
EOL
    ...
    # 101. 安装 allinone & allinone_format 容器
    ...
    if [[ "$choice" -eq 102 ]]; then
        if docker ps -a | grep -q xiaoyaliu/alist; then
            ...
            echo "安装 xiaoya 容器..."
            xiaoya_dir="$software_dir/xiaoya"
            mkdir -p "$xiaoya_dir"
    ...
    if [[ "$choice" -eq 103 ]]; then
        if docker ps -a | grep -q whyour/qinglong; then
            ...
            qinglong_dir="$software_dir/qinglong"
