...
def test_auto_next_game(collector: LogCollector, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(cfg, "GAMEOVER_SCAN_MAX", 3)
    monkeypatch.setattr(an, "GAMEOVER_SCAN_MAX", 3)
    ...
    s.game_over = True
    called: list[int] = []
    s._auto_next_game = lambda: (called.append(1), False)[1]  # type: ignore[method-assign]
    s.move = lambda: True  # type: ignore[method-assign]
    s._running = True
    s._flow()
    ...
    assert dev.taps == [(712, 2198), (712, 2198)], f"应至多点两次，实际 {dev.taps}"
    ...
    s._scan_gameover_interact = lambda: True  # type: ignore[method-assign]

    def fake_wait() -> Any:
        status_seen.append(s._status())
        return _stable_frame(_full_start_board())

    s._wait_for_board_setup = fake_wait  # type: ignore[method-assign]
    ok = s._auto_next_game()
    assert ok is True, collector.logs
    ...
    def interrupt_after_scan(kind: str, msg: str) -> None:
        collector.logs.append(f"[{kind}] {msg}")
        if "开始扫描结算文字" in msg:
            scan_started[0] = True
        if scan_started[0] and len(dev.shells) >= 2:
            s._interrupt.set()

    s, dev = _make_session(collector)
    s._log = interrupt_after_scan  # type: ignore[method-assign]
    s._scan_gameover_text = lambda: hit_queue.pop(0) if hit_queue else None  # type: ignore[method-assign]
    ok = s._auto_next_game()
    assert ok is False, "遮罩不消失应中止"
    assert dev.shells == ["input keyevent 4"] * 3, f"应发3次返回键后中断，{dev.shells}"
