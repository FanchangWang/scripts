...
class StockDataSaver:
    ...
    def get_minute_data(self):
        ...
        try:
            ...
            params = {"symbol": self.stock_symbol, "period": "1d"}
        ...
        try:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    if old_data == data:
                        self.logger.info(
                            f"股票分钟线数据已存在，文件内容相同，无需更新: {file_path}"
                        )
                        self.push_notification(
                            f"股票分钟:📂 {date_str}",
                            f"股票 {self.stock_symbol} 的分钟线数据 {date_str} 已存在，跳过保存",
                        )
                        return True
                    else:
                        self.logger.info(
                            f"股票分钟线数据已存在，文件内容不同，覆盖写入: {file_path}"
                        )
                except Exception as e:
                    self.logger.warning(
                        f"读取文件失败或文件格式错误: {str(e)}，将覆盖写入"
                    )
                ...
                self.push_notification(
                    f"股票分钟:🔄 {date_str}",
                    f"股票 {self.stock_symbol} 的分钟线数据 {date_str} 已存在，🔄 覆盖保存成功",
                )
                return True
            else:
                # 保存数据到文件
                ...
                self.push_notification(
                    f"股票分钟:✅ {date_str}",
                    f"股票 {self.stock_symbol} 的分钟线数据 {date_str} 保存成功，保存路径: {file_path}",
                )
                return True
        except Exception as e:
            ...
            self.push_notification(
                f"股票分钟:❌ {date_str}",
                f"股票 {self.stock_symbol} 的分钟线数据 {date_str} 保存失败，错误信息: {str(e)}",
            )
            return False
    ...
    def run(self):
        ...
        if not data:
            ...
            self.push_notification(
                "股票分钟线数据获取失败",
                f"股票 {self.stock_symbol} 的分钟线数据获取失败",
            )
            return
